function H=Generate_Channel_frequency_selective_LTV(f,TX_pos,RX_pos,Yt,Zt,Yr,Zr,Limitsdeg_t,Limitsdeg_r,dir_max,a,Ts,Tc,v)
...
for ii=1:Ncl 
    ...
    % considering a cluster underground as in equation (5) in article
    % listed above
    ...
    for ll=1:Nray
        ...
        % cluster respect the receiver using equations (3) and (4) in article
        % listed above
        ...
        % Calculation of the length of the path ii-ll as in equations (8)
        ...
        % calculation of delay of the path ii-ll as in equation (9) 
        % in article listed above
...
% Introduction of correlation in time with a correlation matrix as in
% equationn (16) in article listed above
R=zeros(dim_t);
for hh=1:dim_t
    for pp=1:dim_t
...
for t=1:dim_t
    for ii=1:Ncl,
        for ll=1:Nray,
            % Array responses as in equations (12) and (13) in article
            % listed above
            at=Array_response(Yt,Zt,phit(ii,ll),thetat(ii,ll),kd);
            ar=Array_response(Yr,Zr,phir(ii,ll),thetar(ii,ll),kd);
            
            % Lambda Functions as in equations (10) and (11) in article
            % listed above
            ...
            % Calculation of Doppler shift as in equations (17) in article
            % listed above
            v_ms=v*1e3/3600; % express the speed of receiver in m/s
            ni_il=-f*v_ms/3e8*cos(thetar(ii,ll)); % doppler shift
            
            % Calculation of the attenuation of each path as in equation (6)
            % in article listed above
